...
const choose_screen = document.querySelector('.choose-screen');
const sendroomnr_screen = document.querySelector('#sendroomnr');
console.log("ceva");
...
const createGame=()=>{
    choose_screen.classList.remove('active');
...
socket.on("newGame",(data)=>{
    const roomnr = document.querySelector('.roomnr');
    roomnr.textContent = data.room;
    //roomnr = data.room;
    document.cookie = `roomnr=${data.room}`;
});
const joinGame = ()=>{
    choose_screen.classList.remove('active');
    wait.classList.add('active');
...
socket.on("game.begin",(data)=>{
    ...
    sendroomnr_screen.classList.remove('active');
    addButtonEvent();
    //showFriends();
    //initGameGrid();
    //countUpTimer();
...
window.onload = ()=>{
    let len = getCookie('results-lenght');
        for(let i=0; i<len; i++)
        {
            let i_id =getCookie(`${i}_id`);
            let i_profil_image =getCookie(`${i}_profilimage`);
            let i_name =getCookie(`${i}_name`);
            ...
            else
            {
                ...
                profil_picture_image.src = i_profil_image;
...
const sendMessage =  (i)=>{
    let fid = getCookie(`${i}_id`);
    let name = getCookie(`name`);
    let roomnr = getCookie('roomnr');

    let url = `http://localhost:3001/api/send/request/${id}/${fid}/${roomnr}/${name}`;
   ...
   if(ok)
   {
        ...
        .then((res)=>{
            ...
            if(res.ok)
            {
                alert("Invitatia a fost trimisa");
                sendroomnr_screen.classList.remove('active');
                wait.classList.add('active');
            }
        })
...
const showFriends = () =>{
        ...
        wait.classList.remove('active');
        sendroomnr_screen.classList.add('active');
        console.log('aici');
...
const showResult = (nr) =>{
    ...
    if(nr===1)
    {
        ...
        mvf++;
        let action_src = `http://localhost:3001/api/modifyFMultiplayer/${id}/${mvf}`;
    ...
    else
    {
        game_screen.classList.remove('active');
        wait.classList.add('active');
        document.querySelector('#sendroom').style.display = "none";
