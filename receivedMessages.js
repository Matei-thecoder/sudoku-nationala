...
window.onload = () =>{
    ...
    if(receivedmessages == 'nomessages')
    {
        ...
        result_box.textContent = "Nu ai primit niciun mesaj.";
    ...
    else
    {
        ...
        {
            let id= getCookie( `${i}_received_id`);
            let profilimage= getCookie( `${i}_received_profilimage`);
            let name= getCookie( `${i}_received_name`);
            name = decodeURI(name);
            let subiect= getCookie( `${i}_received_subiect`);
            subiect = decodeURI(subiect);
            subiect = decodeURIComponent(subiect);
            let status= getCookie( `${i}_received_status`);
            let messageid = getCookie(`${i}_received_messageid`);
...
const showMesage = (ID) =>{
    ...
    let id = getCookie('id');
    let messageid = getCookie(`${ID}_received_messageid`);
    
    let action_src = `http://localhost:3001/api/view/received/message/${ID}/${friendid}/${messageid}/${id}`;
